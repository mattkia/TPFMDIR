...
def get_dataset(
    ...
    **kwargs
    ...
    if name == 'oasis':
        ...
        target_dataset = CAMUSRegistration
    elif name == 'abdomen':
...
class CAMUSRegistration(RegistrationDataset):
    """Dataset definition of the CAMUS dataset
    """
    def load_image_pair(
        self,
        fixed_id: str,
        ...
        subject_id = fixed_id
        data_path = os.path.join(self.dataset_path, f'database_nifti/{subject_id}')

        fixed_id = f'{subject_id}_2CH_ES'
        moving_id = f'{subject_id}_2CH_ED'

        fixed_img = nib.load(os.path.join(data_path, f'{fixed_id}.nii.gz')).get_fdata()
        moving_img = nib.load(os.path.join(data_path, f'{moving_id}.nii.gz')).get_fdata()
        
        fixed_seg = nib.load(os.path.join(data_path, f'{fixed_id}_gt.nii.gz')).get_fdata()
        moving_seg = nib.load(os.path.join(data_path, f'{moving_id}_gt.nii.gz')).get_fdata()

        fixed_img = resampler_sitk(fixed_img, spacing=(1, 1), dimension=2)
        moving_img = resampler_sitk(moving_img, spacing=(1, 1), dimension=2)

        fixed_img = pad_and_resize(fixed_img, (self.crop_x, self.crop_y))
        moving_img = pad_and_resize(moving_img, (self.crop_x, self.crop_y))

        fixed_seg = pad_and_resize(fixed_seg, (self.crop_x, self.crop_y), order=0)
        moving_seg = pad_and_resize(moving_seg, (self.crop_x, self.crop_y), order=0)

        return fixed_img, moving_img, fixed_seg, moving_seg

    def setup_fixed_moving_ids(self):
        if os.path.exists('tmp/camus_train_val_test.json'):
            # Load the existing information
            with open('tmp/camus_train_val_test.json', 'r') as handle:
                return json.load(handle)[self.mode]
            
        else:
            split_info_path = os.path.join(self.dataset_path, 'database_split')
            with open(os.path.join(split_info_path, 'subgroup_training.txt'), 'r') as handle:
                training_subjects = handle.readlines()
                training_subjects = list(map(lambda x: x.strip(), training_subjects))[:190]
            with open(os.path.join(split_info_path, 'subgroup_validation.txt'), 'r') as handle:
                val_subjects = handle.readlines()
                val_subjects = list(map(lambda x: x.strip(), val_subjects))
            with open(os.path.join(split_info_path, 'subgroup_testing.txt'), 'r') as handle:
                test_subjects = handle.readlines()
                test_subjects = list(map(lambda x: x.strip(), test_subjects))

            train_pairs = [(subject, subject) for subject in training_subjects]
            val_pairs = [(subject, subject) for subject in val_subjects]
            test_pairs = [(subject, subject) for subject in test_subjects]

            camus_train_val_test = {'train': train_pairs,
                                    'val': val_pairs,
                                    'test': test_pairs}
            
            os.makedirs('tmp', exist_ok=True)
            with open('tmp/camus_train_val_test.json', 'w') as handle:
                json.dump(camus_train_val_test, handle)
            
            return camus_train_val_test[self.mode]

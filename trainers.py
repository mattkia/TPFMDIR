...
from visualization import (save_grid,
                           ...
                           animate_warping,
                           save_vector_field,
                           save_velocity_quivers,
                           save_velocity_heatmaps,
                           save_grid_overlayed_image)
...
class DiceTrainer:
    ...
    def __init__(self, config: Dict):
        ...
        self.num_steps = training_config.get('nfe', 8)
    ...
    def run(self):
        print('[*] Training on Continuous Cocycle Regularization...')
        self.logger.debug('Training with cocycle regularization started...')
        ...
        for epoch in iters:
            ...
            for sample in self.loader:
                ...
                for level in range(self.res_levels):
                    ...
                    loss, cocycle_loss = self.network.loss_flow(fixed, moving, grid, res[level])
                    
                    total_loss = self.image_loss_coeff * loss + self.cocycle_loss_coeff * cocycle_loss

                    total_loss.backward()
                    self.optimizer.step()

                    avg_img_loss += loss.item()
                    avg_cocycle_loss += cocycle_loss.item()
            ...
            avg_cocycle_loss /= (self.res_levels * len(self.loader))
            ...
            if epoch % self.eval_interval == 0 or epoch == self.epochs - 1:
                ...
                log = f'Epoch:{epoch}, Image Loss: {avg_img_loss:.4f}, Contraint Loss: {avg_cocycle_loss * self.cocycle_loss_coeff:.4f}, '
...
class DiceTester:
    def __init__(self, config: Dict):
        ...
        self.num_steps = test_config.get('nfe')
    ...
    def run(self):
        ...
        for batch_id, sample in enumerate(tqdm(self.loader)):
            ...
            if self.save_evals:
                ...
                save_velocity_heatmaps(self.network,
                                       fixed,
                                       moving,
                                       os.path.join(batch_path, 'velocities.png'))
                save_velocity_quivers(self.network,
                                      fixed,
                                      moving,
                                      os.path.join(batch_path, 'arrows.png'))
        ...
        print('Evaluating Cocycle Property...')
...
class TRETester:
    ...
    def run(self):
        ...
        for batch_id, sample in enumerate(tqdm(self.loader)): 
            ...
            forward_sdlogj = self.sdlogj(forward_deformation)
            backward_sdlogj = self.sdlogj(backward_deformation)
            ...
            avg_forward_sdlogj.append(forward_sdlogj.item())
            avg_backward_sdlogj.append(backward_sdlogj.item())
